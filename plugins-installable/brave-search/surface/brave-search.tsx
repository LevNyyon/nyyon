...
export default function BraveSearch() {
  ...
  const doConnect = async () => {
    ...
    try {
      ...
      if (r.ok) { setNote(r.note || 'Connected.'); setKey(''); } else setErr(r.error || 'Brave refused the key');
      await load();
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto p-6 max-w-xl space-y-4">
      <header>
        <h1 className="text-lg font-semibold">Brave Search</h1>
        <p className="text-[12px] text-mute mt-1 leading-relaxed">
          Web search for Nyo and the Digest. Needs a Brave Search API key (free tier). The key lives in this plugin's own table.
        </p>
      </header>
      ...
      <div className="hairline rounded-sm p-4 space-y-2.5">
        {status?.connected ? (
          <>
            <div className="text-[13px]"><span className="text-emerald-700 dark:text-emerald-400">Connected</span></div>
          ...
          <>
            <ol className="text-[12px] text-mute leading-relaxed pl-4 list-decimal space-y-1">
              <li><a href="https://api.search.brave.com/app/keys" target="_blank" rel="noreferrer" className="underline hover:text-ink">Create a free Brave Search API key</a>.</li>
              <li>Paste it here. It is verified with a real query before it is stored.</li>
            </ol>
            <div className="flex gap-2">
              <input type="password" value={key} onChange={(e) => setKey(e.target.value)} placeholder="BSA..."
              ...
              <button onClick={() => void doConnect()} disabled={busy || !key.trim()}
                className="text-[12px] px-3 rounded-sm bg-ink text-paper disabled:opacity-40">{busy ? 'connecting…' : 'connect'}</button>
            </div>
          </>
        )}
      </div>
