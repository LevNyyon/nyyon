// Editorial plugin — brain. Ported from workers/api/src/lib/brain.js.
// The Sunday Brain: a weekly editorial planning interview that turns the
// operator's knowledge, opinions, and live commentary into the coming week's
// article slate.
//
// Flow:
//   1. Sunday (or on demand) → startBrainSession: reads the knowledge base +
//      recent work, generates ~18 questions, saves a brain_sessions row
//      (status='open'), returns the questions for Nyo to ask.
//   2. Operator answers in chat (free text — all at once or across messages).
//   3. submitBrainAnswers: saves the answers, derives 5-8 article ideas, and
//      creates an aeo_questions row for each with interview_status='ready' +
//      the brain answers as expert_context (the brain IS the interview).
//
// Plugin capability contract: this file imports NOTHING; every exported fn
// takes `api` first. Shared helpers (aeo_questions store, blog post listing,
// taste/pulse/top-signals reads) are duplicated inline — lib files cannot
// import each other.
//
// Tables:    plugin_editorial_brain_sessions, plugin_editorial_aeo_questions,
//            plugin_editorial_blog_posts (read), plugin_editorial_osint_signals (read)
// Knowledge: reads host brand, brand-voice; reads own
...
// ─── inline duplicates: blog posts listing (see blog-db.mjs) ────────────────
...
// ─── inline duplicates: taste + heartbeat pulse/top signals ─────────────────
async function readTasteProfile(api) {
  const doc = await api.knowledge('plugin-editorial-editorial-taste').catch(() => null);
  return doc?.body || null;
}

async function readPulse(api) {
  const doc = await api.knowledge('plugin-editorial-industry-pulse').catch(() => null);
  return doc?.body || null;
}

// Compacted from heartbeat's topSignals for the one call the brain makes
// (no free-text term — the brain never passes `q`).
async function topSignalRows(api, { days = 7, minContent = 55, limit = 12 } = {}) {
...
// ─── week math ──────────────────────────────────────────────────────────────
// Returns ms-epoch of the most recent Sunday at 00:00 UTC. Workers run in UTC;
// we don't have the operator's tz, so UTC Sunday — close enough for a weekly
// cadence, and consistent.
...
export async function getThisWeekSession(api) {
  const week = weekOfSunday();
  return api.db.prepare(`SELECT * FROM plugin_editorial_brain_sessions WHERE week_of = ? ORDER BY created_at DESC LIMIT 1`)
    .bind(week).first();
}

export async function getBrainSession(api, id) {
  return api.db.prepare(`SELECT * FROM plugin_editorial_brain_sessions WHERE id = ?`).bind(id).first();
}

export async function recentBrainSessions(api, { limit = 8 } = {}) {
  const r = await api.db.prepare(`SELECT * FROM plugin_editorial_brain_sessions ORDER BY week_of DESC LIMIT ?`).bind(limit).all();
  return r.results || [];
}
...
export async function generateBrainQuestions(api) {
  const [brand, voice, recentPosts, taste] = await Promise.all([
    api.knowledge('plugin-editorial-brand').catch(() => null),
  ...
  try {
    pulse = await readPulse(api);
    liveSignals = await topSignalRows(api, { days: 7, minContent: 65, limit: 8 });
  ...
  const raw = String(await api.gateway('llm', 'text', { system: QUESTION_SYSTEM, prompt, model: 'gpt-4o-mini' }));
...
export async function startBrainSession(api, { force = false } = {}) {
  const existing = await getThisWeekSession(api);
  if (existing && !force) {
    return { ...existing, questions: safeArr(existing.questions_json), reused: true };
  }
  const questions = await generateBrainQuestions(api);
  const id = uid();
  const t = now();
  await api.db.prepare(
    `INSERT INTO plugin_editorial_brain_sessions (id, week_of, status, questions_json, created_at) VALUES (?, ?, 'open', ?, ?)`,
  ).bind(id, weekOfSunday(), JSON.stringify(questions), t).run();
  try { await api.log('brain_session_started', { id, week_of: weekOfSunday() }); } catch { /* never fatal */ }
...
export async function deriveArticles(api, sessionId) {
  const session = await getBrainSession(api, sessionId);
  ...
  const out = await api.gateway('llm', 'json', { system: DERIVE_SYSTEM, prompt });
  ...
  await api.db.prepare(
    `UPDATE plugin_editorial_brain_sessions SET status='derived', derived_json=?, completed_at=? WHERE id=?`,
  ).bind(JSON.stringify(created), now(), sessionId).run();
  try { await api.log('brain_derived', { session_id: sessionId, articles: created.length }); } catch { /* never fatal */ }
  ...
  try {
    knowledgeSlug = await archiveBrainToKnowledge(api, { session, questions, ideas: created });
  } catch {
...
// week's themes, the founder's key positions, and the article slate. The docs
// live in the plugin namespace (plugin-editorial-brain-archive parent +
// plugin-editorial-brain-<date> children) so the whole editorial history is
// browsable and future question/article generation can draw on accumulated POV.
...
async function archiveBrainToKnowledge(api, { session, questions, ideas }) {
  const weekDate = new Date(session.week_of).toISOString().slice(0, 10);
  const slug = `plugin-editorial-brain-${weekDate}`;
  ...
  const md = String(await api.gateway('llm', 'text', { system: ARCHIVE_SYSTEM, prompt, model: 'gpt-4o-mini' }));

  // Ensure the parent archive doc exists (idempotent upsert).
  await api.saveKnowledge('plugin-editorial-brain-archive', {
  ...
  await api.saveKnowledge(slug, {
    title: `Brain · Week of ${weekDate}`,
    body: md,
  });
  await api.log('brain_archived', { slug, week_of: session.week_of });

  return slug;
}

export async function submitBrainAnswers(api, { sessionId, answers }) {
  let session = sessionId ? await getBrainSession(api, sessionId) : await getThisWeekSession(api);
  ...
  if (!session) session = await startBrainSession(api, {});
  await api.db.prepare(`UPDATE plugin_editorial_brain_sessions SET answers_text = ? WHERE id = ?`)
    .bind(answers, session.id).run();
  return deriveArticles(api, session.id);
}
...
async function ensureUniqueQuestionSlug(api, base) {
  if (!(await readAeoQuestionRow(api, base))) return base;
  for (let i = 2; i <= 50; i++) {
    const next = `${base}-${i}`;
    if (!(await readAeoQuestionRow(api, next))) return next;
  }
