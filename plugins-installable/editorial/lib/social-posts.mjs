...
export const CHANNELS = [
  { key: 'linkedin-company',  network: 'LinkedIn', voice: 'plugin-editorial-brand',    label: 'LinkedIn (company page)' },
  { key: 'facebook-company',  network: 'Facebook', voice: 'plugin-editorial-brand',    label: 'Facebook (company page)' },
...
export async function draftSocialPostText(api, channelKey, { title, excerpt = null, tags = null, url, bodyHtml = '', sourceKind = 'blog' } = {}) {
  ...
  const [brandVoice, personalVoice, styleRules] = await Promise.all([
    ...
    api.knowledge('plugin-editorial-personal-voice').catch(() => null),
    api.knowledge('plugin-editorial-writing-style-rules').catch(() => null),
  ]);
