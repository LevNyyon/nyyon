...
// Gateways:  llm(text, json), outbox(begin, sent, failed), calendar(upsert),
//            render(cover, images).
//
...
// - Article figures: the resvg/font renderer stays HOST behind the render
//   gateway. The full 2-5 in-body diagram set is generated by the pack's
//   figures lib/tools; inside the writer the best-effort visuals step renders
//   the branded FEATURED COVER (render:cover) and, if that fails, one AI
//   featured-image candidate (render:images). Neither ever blocks the article.
  ...
  if (published && published_at) {
    ...
    try {
      await api.gateway('calendar', 'upsert', {
        kind: 'blog_publish', title, description: excerpt, starts_at: published_at, all_day: true,
        status: published_at <= now() ? 'done' : 'confirmed',
        source: 'blog', source_ref: slug, link_url: `/blog/${slug}`,
      ...
      await api.log('workflow_run', {
        ...
        trigger_payload: { blog_slug: slug, title }, error: String(e?.message || e), started_at: startedAt,
      });
      throw e; // calendar mirror failure shouldn't be silent
    }
  }
...
// ─── best-effort visuals: branded cover via the render gateway ──────────────
// The host generateArticleFigures (LLM-picked diagram set + embed + cover)
// lives with the pack's figures lib; the writer's inline visuals step renders
// the deterministic branded cover and points the post's featured image at it.
...
async function generateCoverFigure(api, { slug, actor = 'system' } = {}) {
  ...
  } catch { /* tags may be a plain string or null */ }

  // Draft the three hero slots (cheap LLM step, best-effort — deterministic
  // fallback slots mean a failed draft still yields a real cover).
  let cover = null;
  try {
    ...
    if (parsed && typeof parsed === 'object') {
      cover = { kicker: parsed.kicker || null, highlight: parsed.highlight || '', sub: parsed.sub || null };
    }
  } catch { /* deterministic slots below */ }

  // Render on the host (resvg WASM + fonts + R2 stay behind the gateway).
  // Title/excerpt/cover are passed explicitly so the renderer needs no table
  // read of its own.
  const r = await api.gateway('render', 'cover', {
    title: post.title,
    excerpt: post.excerpt || '',
    cover: {
  ...
  if (!url) throw new Error('render:cover returned no cover_url');
  ...
  ).bind(url, 'article-cover', t, t, actor, slug).run();
  await api.log('article_cover_generated', { slug, featured_url: url });
  return { ok: true, slug, count: 0, figures: [], featured_url: url, cover_only: true };
}

// AI featured-image fallback (fires only when the cover render failed).
// Ported compactly from blog-images.js: brand style block + brief drafter, ONE
// candidate through render:images (no vision judge — one candidate needs none).
...
async function generateFeaturedImageFallback(api, { slug, title, excerpt, tags, actor = 'system' } = {}) {
  let scene = null, label = null;
  try {
    const raw = String(await api.gateway('llm', 'text', {
      system: BRIEF_SYSTEM,
      prompt: [
        `Title: ${title}`,
    ...
    scene = (sceneMatch?.[1] || raw).trim().replace(/^["']|["']$/g, '');
    label = labelMatch?.[1]?.trim().replace(/^["']|["']$/g, '') || null;
  } catch {
  ...
  const labelBar = label
    ? ` At the very bottom: a thin white horizontal line spanning 70% of the width, small dot on far left labeled "${label.split('/')[0].trim()}" in clean white sans-serif caps, small dot on far right labeled "${label.split('/')[1] ? label.split('/')[1].trim() : ''}" in clean white sans-serif caps. Minimal, understated.`
    : '';
  const prompt = `${NYYON_STYLE_BLOCK} ${scene}${labelBar}`;

  const r = await api.gateway('render', 'images', { blog_slug: slug, prompt, n: 1 });
  const cand = Array.isArray(r?.candidates) ? r.candidates[0] : null;
  if (!cand?.url) throw new Error('render:images produced no candidate');

  const generatedAt = now();
  const versionedUrl = `${cand.url}${cand.url.includes('?') ? '&' : '?'}v=${generatedAt}`;
  await api.db.prepare(
    `UPDATE plugin_editorial_blog_posts SET featured_image_url=?, featured_image_prompt=?, featured_image_model=?, featured_image_generated_at=?, updated_at=?, updated_by=? WHERE slug=?`,
  ).bind(versionedUrl, prompt, cand.model || null, generatedAt, generatedAt, actor, slug).run();
  await api.log('blog_image_generated', { slug, model: cand.model || null, size_bytes: cand.size_bytes || null });
  return { url: versionedUrl, model: cand.model || null, size_bytes: cand.size_bytes || null };
}

// Best-effort visuals for a freshly saved article: branded cover first, AI
// image only if the cover path failed. Mirrors the original's "figures then
// image fallback" contract; never throws.
async function bestEffortVisuals(api, { slug, actor }) {
  let figures = null, image = null;
  try {
    figures = await generateCoverFigure(api, { slug, actor });
  } catch (e) {
    try { await api.log('article_figures_failed', { slug, error: String(e?.message || e).slice(0, 300) }); } catch { /* */ }
  }
  if (figures?.featured_url) {
    ...
    try {
      ...
      const img = await generateFeaturedImageFallback(api, {
        slug, title: post?.title || 'Untitled', excerpt: post?.excerpt, tags, actor,
      });
...
export async function readVoiceProfile(api, { voice = 'house' } = {}) {
  ...
  if (voice === 'personal') {
    const personalDoc = await api.knowledge('personal-voice').catch(() => null);
...
export async function composeAndSavePost(api, {
  ...
  // Visuals — best-effort: never block the article from saving.
  const { figures, image } = await bestEffortVisuals(api, { slug: finalSlug, actor });
...
export async function expandPostWithFaq(api, { slug, voice = 'personal', actor = 'operator' } = {}) {
  ...
  // 1. Save the expanded prose. writeBlogPostRow leaves featured_image_url intact.
  await writeBlogPostRow(api, {
    slug,
    title:        post.title,
  ...
  // 2. Refresh the cover for the expanded content (best-effort).
  let figures = null;
  try {
    figures = await generateCoverFigure(api, { slug, actor });
  } catch (e) {
...
export async function runAeoCron(api, { actor = 'aeo-cron', targetSlug = null, readyOnly = false } = {}) {
  ...
  try {
    ...
    // Visuals — best-effort: branded cover (render gateway), AI-image
    // fallback. Never rolls back the saved article.
    const { figures, image } = await bestEffortVisuals(api, { slug: safeSlug, actor });
    ...
    const figLine  = figures?.featured_url ? ` Rendered the branded cover.` : '';
    ...
    return { ok: true, question_slug: q.slug, blog_slug: safeSlug, title: draft.title, figures, image, deployed: deploy.ok, deploy_reason: deploy.reason || null, nyo_message: nyo.content };
  } catch (e) {
