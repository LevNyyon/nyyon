...
// bubble is logged to gtm_sends (and rides lib/whatsapp.js sendText → outbox).

import { logEvent } from './db.js';
import { sendText, toChatId, backfillMessages, ensureListening } from './whatsapp.js';
import { getLead, gtmLLM, extractJson } from './gtm.js';
import { gtmDoc, readYou, listOrgPeople, greenLeads } from './gtm-context.js';
...
export async function draftAnglesForLead(env, { lead, org_people = [] } = {}) {
...
// The old fat entry point, kept working for the routes that still call it:
...
export async function generateAngles(env, leadId) {
  ...
  const r = await draftAnglesForLead(env, { lead, org_people: people });
  ...
  await saveAngles(env, leadId, r.angles_payload);
  await logEvent(env, { kind: 'gtm_angles_generated', actor: 'operator', payload: { id: leadId, angles: r.angles_payload.angles.length } });
...
export async function greenLeadsWithStatus(env) {
  const leads = await greenLeads(env);
  const ids = leads.map((l) => l.id);
  const [angles, statuses] = await Promise.all([readAnglesMany(env, ids), contactStatuses(env, ids)]);
...
// length, capped. The gtm-outreach knowledge doc has always CLAIMED pacing is
// tunable there; now it actually is — add a fenced ```json block containing
// {"pacing": {"gap_min_ms":4000, "gap_jitter_ms":5000, "cap_ms":12000}} to the
// doc and the next send uses it. Defaults preserve the shipped 4-9s/12s feel.
const PACING_DEFAULTS = { gap_min_ms: 4000, gap_jitter_ms: 5000, cap_ms: 12000 };
async function pacingConfig(env) {
  try {
    const { readKnowledge } = await import('./db.js');
    const doc = await readKnowledge(env, 'gtm-outreach');
...
export async function sendOutreach(env, { lead_id, bubbles, force = false, source = 'operator', source_ref = null } = {}) {
  ...
  if (!force) {
    const recent = await env.DB.prepare("SELECT COUNT(*) AS n FROM gtm_sends WHERE lead_id = ? AND status = 'sent' AND created_at > ?")
      .bind(lead_id, now() - 10 * 60 * 1000).first();
    if ((recent?.n ?? 0) > 0) {
      return { error: 'bubbles were already sent to this lead in the last 10 minutes — the earlier send very likely went through (check gtm_sends / the Outbox). Pass force:true only if the operator confirms a re-send.' };
    }
  }
  const chatId = toChatId(lead.normalized_phone || lead.phone);
  await ensureListening(env, chatId).catch(() => {});
  const pacing = await pacingConfig(env);
  const sent = [];
  for (let i = 0; i < msgs.length; i++) {
    try {
      // sendText throws on failure (it never returns a soft error) — the catch
      // below is the single failure path.
      await sendText(env, { chatId, text: msgs[i] }, { source, source_ref });
      await env.DB.prepare('INSERT INTO gtm_sends (id, lead_id, chat_id, bubble, status, error, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)')
  ...
  await logEvent(env, { kind: 'gtm_outreach_sent', actor: 'operator', payload: { id: lead_id, bubbles: msgs.length, chat_id: chatId } });
...
export async function leadThread(env, leadId, { refresh = false } = {}) {
  ...
  if (refresh && chatIds.length) {
    for (const cid of chatIds) {
      try { await backfillMessages(env, { chatId: cid, allAutoListen: false, limit: 100 }); }
