...
// the SQL for hot_take_packages (and for the package's legs in the unified
// social_posts table) lives, so both the /api/hot-takes routes AND the Hot Takes
// tools delegate here and every mutation logs to the activity bus once. This lib does DB + orchestration only; NEW
...
import { listConnections, postToConnection } from './social-gateway.js';
...
// Legs live in the UNIFIED social_posts table (migration 0062 folded
// hot_take_posts into it): one store for every social post, package_id linking
// the Hot Takes legs. The column is `content` there, but every reader in this
// file, in the routes and in the HotTakes page speaks `body` — so reads alias
// it back rather than forcing a rename through four layers.
const POST_SELECT = 'SELECT *, content AS body FROM social_posts';

export async function listPosts(env, packageId) {
  const r = await env.DB.prepare(`${POST_SELECT} WHERE package_id = ? ORDER BY channel ASC`)
    .bind(packageId).all();
  return r.results || [];
}

export async function readPost(env, id) {
  return env.DB.prepare(`${POST_SELECT} WHERE id = ?`).bind(id).first();
}
...
export async function upsertPost(env, { package_id, channel, body, notes, image_url, status, scheduled_at, actor = 'hot-takes' } = {}) {
  ...
  const existing = await env.DB.prepare(
    'SELECT id FROM social_posts WHERE package_id = ? AND channel = ?',
  ...
  const pkg = await env.DB.prepare('SELECT blog_slug FROM hot_take_packages WHERE id = ?').bind(package_id).first();
  const id = genId('htp');
  await env.DB.prepare(
    `INSERT INTO social_posts (id, blog_slug, package_id, channel, content, notes, image_url, status, scheduled_at, actor, created_at, updated_at)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
...
export async function patchPost(env, id, patch = {}, actor = 'operator') {
  ...
  args.push(now());
  args.push(id);
  await env.DB.prepare(`UPDATE social_posts SET ${fields.join(', ')} WHERE id = ?`).bind(...args).run();
...
export async function buildArticleSeed(env, id) {
...
export async function linkArticle(env, id, { slug, title, excerpt } = {}, actor = 'operator') {
  ...
  const post = (title === undefined || excerpt === undefined) ? await readBlogPost(env, slug).catch(() => null) : null;
  const updated = await patchPackage(env, id, {
    blog_slug: slug,
  ...
  await env.DB.prepare('UPDATE social_posts SET blog_slug = ? WHERE package_id = ? AND blog_slug IS NULL')
    .bind(slug, id).run();
  await logEvent(env, { kind: 'hottake_article_linked', actor, payload: { id, slug } });
  return updated;
}
...
export async function writeArticleFromBrief(env, id, { voice = 'house', actor = 'operator' } = {}) {
  ...
  await linkArticle(env, id, { slug: res.slug, title: res.title || seed.title, excerpt: res.post?.excerpt || null }, actor);
...
export async function publishWebsite(env, id, { actor = 'operator', ctx = null } = {}) {
  ...
  const { publishBlogPostToProd } = await import('./publish.js');
  const r = await publishBlogPostToProd(env, pkg.blog_slug, { source: 'hottake', ctx, social: false });
  if (r?.ok) {
...
export async function postLeg(env, postId, { actor = 'operator' } = {}) {
  ...
  try {
    const res = await postToConnection(env, post.channel, {
...
export async function runDueReleases(env, { ctx = null } = {}) {
  ...
  const duePosts = await env.DB.prepare(
    `SELECT p.id, p.package_id FROM social_posts p
...
export async function searchHotTakes(env, { q = '', limit = 30 } = {}) {
  ...
  const po = await env.DB.prepare(
    ...
    `SELECT p.id, p.package_id, p.channel, p.status, SUBSTR(COALESCE(p.content,''),1,140) AS snippet
       FROM social_posts p WHERE p.package_id IS NOT NULL
