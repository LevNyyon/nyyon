// Outreach · WA — the prospect conversation surface (nyyon-lite layer 2 impl).
//
...
//
// The lead <-> chat join is DERIVED, not stored: gtm_leads.normalized_phone maps
// to the WhatsApp DM id `<digits>@c.us` (lib/whatsapp.js toChatId). No new table.
//
// Everything the operator might want to tune — how many threads, how much
// history, how the draft is written — lives in the `outreach-reply-drafting`
// knowledge doc, seeded on first read. The model is reached ONLY through the
// `llm` gateway.
...
import { recentMessages, toChatId, syncFromGateway } from './whatsapp.js';
import { getThreadStats } from './outreach-threads.js';
import { readAngles } from './gtm-outreach.js';
import { getLead, isCompanyLinkedin, leadState } from './gtm.js';
import { loadCohortCadence } from './outreach-cadence.js';

const DRAFT_SLUG = 'outreach-reply-drafting';
...
const FIRST_TOUCH_SLUG = 'outreach-first-touch';
...
export async function loadFirstTouchTemplate(env) {
  try {
    const doc = await readKnowledge(env, FIRST_TOUCH_SLUG);
    if (!doc) {
      await writeKnowledge(env, {
        slug: FIRST_TOUCH_SLUG, title: 'Outreach · default first touch',
        body: firstTouchSeedBody(FIRST_TOUCH_DEFAULT), parent_slug: 'module-outreach',
      }).catch(() => {});
...
export async function loadDraftingRules(env) {
  try {
    const doc = await readKnowledge(env, DRAFT_SLUG);
    if (!doc) {
      await writeKnowledge(env, {
        slug: DRAFT_SLUG, title: 'Outreach · WA reply drafting',
        body: draftSeedBody(DRAFT_DEFAULT, DRAFT_LIMITS), parent_slug: 'module-outreach',
      }).catch(() => {});
...
export async function saveDraftingRules(env, { rules, limits } = {}) {
  const cur = await loadDraftingRules(env);
  ...
  await writeKnowledge(env, {
    slug: DRAFT_SLUG, title: 'Outreach · WA reply drafting',
    body: draftSeedBody(clean, nextLimits), parent_slug: 'module-outreach',
  });
  await logEvent(env, { kind: 'outreach_drafting_rules_updated', payload: { chars: clean.length, limits: nextLimits } });
...
function prospectCard(lead) {
  ...
  return {
    ...
    // linkedin_company entry (gtm.js manualEditLead owns that shape).
...
async function digitsForLids(env, lids = []) {
  const out = new Map();
...
async function leadsByChatId(env, chatIds = []) {
  const ids = chatIds.map(String);
  const lidResolved = await digitsForLids(env, ids.filter((id) => id.endsWith('@lid')));
...
async function lastMessages(env, chatIds = []) {
  const out = new Map();
...
async function deadMarks(env, leadIds = []) {
  const out = new Map();
  for (let i = 0; i < leadIds.length; i += CHUNK) {
    const c = leadIds.slice(i, i + CHUNK);
    const r = (await env.DB.prepare(
      `SELECT lead_id, dead_at, dead_reason FROM outreach_conversation_state
...
export async function conversationStatesFor(env, leads = []) {
  const out = new Map();
  if (!leads.length) return out;
  // Reality first, exactly as the inbox does before IT reads. Without this the
  // parity above is a claim rather than a fact: a reply that arrived since the
  // last cron would read `active` in Conversations and `touched` here — and the
  // approve button would happily offer to message someone mid-conversation.
  // Incremental (created_at cursor), so this is a handful of rows.
  await syncFromGateway(env).catch(() => {});
  const deadAfterDays = (await loadCohortCadence(env)).cadence.dead_after_days;
  ...
  const [lasts, marks] = await Promise.all([
    lastMessages(env, everyId).catch(() => new Map()),
    deadMarks(env, leads.map((l) => l.id)).catch(() => new Map()),
  ]);
...
export async function setConversationDead(env, { lead_id, dead = true, reason = null } = {}) {
  if (!lead_id) return { error: 'lead_id required' };
  const t = now();
  await env.DB.prepare(
    `INSERT INTO outreach_conversation_state (lead_id, dead_at, dead_reason, updated_at)
     VALUES (?, ?, ?, ?)
  ...
  await logEvent(env, {
    kind: dead ? 'outreach_conversation_marked_dead' : 'outreach_conversation_revived',
    payload: { lead_id, reason: reason || null },
  });
...
export async function listProspectThreads(env, { q = '', limit = null, status = 'active' } = {}) {
  // Freshness is the ROUTE's job (background waitUntil on the gateway sync)
  // so this read answers straight from D1 — blocking here doubled the
  // module's first paint.
  const { limits } = await loadDraftingRules(env);
  ...
  const fromChats = ((await env.DB.prepare(
  ...
  const leads = await leadsByChatId(env, ids);
  ...
  const [lasts, stats, marks] = await Promise.all([
    lastMessages(env, matchedIds),
    getThreadStats(env, matchedIds).catch(() => new Map()),
    deadMarks(env, [...new Set(matched.map((c) => leads.get(c.id).id))]),
  ]);
  ...
  // are never touched, a fresh row appears only when the lead matched NO chat.
  {
    ...
    const allLeads = (await env.DB.prepare(
      `SELECT * FROM gtm_leads WHERE normalized_phone IS NOT NULL ORDER BY updated_at DESC LIMIT 400`,
    ).all().catch(() => ({ results: [] }))).results || [];
  ...
  // the queued text and a 'scheduled' status until it fires.
  {
    const live = (await env.DB.prepare(
      `SELECT lead_id, bubbles, send_at FROM scheduled_sends WHERE status IN ('scheduled','claimed') ORDER BY send_at ASC LIMIT 200`,
    ).all().catch(() => ({ results: [] }))).results || [];
...
// A row we wrote ourselves at send time (lib/whatsapp.js preInsertOutbound
// stamps raw_json.source), as opposed to one the gateway's webhook delivered.
// recentMessages parses raw_json, so accept either shape.
...
async function siblingChatIds(env, chatId, lead) {
  ...
  try {
    if (String(chatId).endsWith('@lid')) {
      const r = await env.DB.prepare('SELECT phone, pn FROM wa_lid_map WHERE lid = ?').bind(chatId).first();
      const d = digitsOf(r?.phone || String(r?.pn || '').split('@')[0]);
      if (d) ids.add(`${d}@c.us`);
    }
    for (const pn of [...ids].filter((i) => String(i).endsWith('@c.us'))) {
      const r = await env.DB.prepare('SELECT lid FROM wa_lid_map WHERE pn = ?').bind(pn).all();
...
export async function threadFacts(env, { chat_id, lead = null, limit = 300 } = {}) {
  const chatIds = await siblingChatIds(env, chat_id, lead);
  const rows = (await Promise.all(
    chatIds.map((cid) => recentMessages(env, { chat_id: cid, limit }).catch(() => [])),
  )).flat();
...
export async function readProspectThread(env, { chat_id, lead_id = null, limit = null } = {}) {
  const { limits } = await loadDraftingRules(env);
  let chatId = chat_id || null;
  let lead = null;

  if (!chatId && lead_id) {
    lead = await getLead(env, lead_id).catch(() => null);
  ...
  if (!lead) {
    const found = await leadsByChatId(env, [chatId]);
    lead = found.get(chatId) || null;
  }

  const { chat_ids: chatIds, messages: merged, answered } = await threadFacts(env, {
  ...
  const stats = await getThreadStats(env, chatIds).catch(() => new Map());
...
export async function draftReply(env, { chat_id, lead_id = null, force_llm = false } = {}) {
  const { rules, limits } = await loadDraftingRules(env);
  let chatId = chat_id || null;
  let lead = lead_id ? await getLead(env, lead_id).catch(() => null) : null;
  ...
  const angles = await readAngles(env, lead.id).catch(() => null);
  ...
  const chatIds = await siblingChatIds(env, chatId, lead);
  const rows = (await Promise.all(
    chatIds.map((cid) => recentMessages(env, { chat_id: cid, limit: limits.draft_context_messages }).catch(() => [])),
  )).flat();
  ...
  if (!theySpoke && !force_llm) {
    ...
    if (!bubbles.length) {
      ...
      // outreach-first-touch doc, personalised, straight into the panel.
      const template = await loadFirstTouchTemplate(env);
      return {
    ...
    await logEvent(env, {
      kind: 'outreach_draft_suggested',
      payload: { lead_id: lead.id, chat_id: chatId, source: 'angle', step: sentCount },
    });
    return {
...
${rules}`;
  ...
  await logEvent(env, { kind: 'outreach_draft_suggested', payload: { lead_id: lead.id, chat_id: chatId, source: 'llm', chars: draft.length } });
  return {
...
export async function pickNextBubble(env, {
  ...
  await logEvent(env, { kind: 'outreach_draft_suggested', payload: { lead_id: leadId, source: 'angle', step: sentCount } });
  return {
...
export async function composeReply(env, {
  ...
  const cfg = {
    rules: typeof rules === 'string' && rules.trim() ? rules : (await loadDraftingRules(env)).rules,
    limits: limits && typeof limits === 'object' ? { ...DRAFT_LIMITS, ...limits } : (await loadDraftingRules(env)).limits,
  };
  const card = prospect || {};
  const history = Array.isArray(messages) ? messages : [];
  const theySpoke = answered === null || answered === undefined
...
${cfg.rules}`;
  ...
  await logEvent(env, {
    kind: 'outreach_draft_suggested',
    payload: { lead_id: card.lead_id || null, source: 'llm', chars: composed.length },
  });
  return {
